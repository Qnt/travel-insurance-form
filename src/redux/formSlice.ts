...
import type { Activity, Country, FormData, Program } from '../types';
...
const savedFormData = localStorage.getItem('travel-insurance-form-data');
const parsedFormData = savedFormData
  ? (JSON.parse(savedFormData) as FormData)
  : initialState.formData;

const savedCurrentStepIndex = localStorage.getItem(
  'travel-insurance-form-step-index'
);
const parsedCurrentStepIndex = savedCurrentStepIndex
  ? JSON.parse(savedCurrentStepIndex)
...
const formSlice = createSlice({
  ...
  initialState: {
    ...
    currentStepIndex: parsedCurrentStepIndex,
  },
  reducers: {
    updateFields: (state, action: PayloadAction<Partial<FormData>>) => {
      state.formData = { ...state.formData, ...action.payload };
      localStorage.setItem(
        'travel-insurance-form-data',
        JSON.stringify(state.formData)
      );
    },
    nextStep: state => {
      if (state.currentStepIndex < 2) state.currentStepIndex += 1;
      localStorage.setItem(
        'travel-insurance-form-step-index',
        JSON.stringify(state.currentStepIndex)
      );
    },
    prevStep: state => {
...
export const { updateFields, nextStep, prevStep } = formSlice.actions;
